...
Usage:
Import the module using `import benchmark_utils` and then access the functions
as `benchmark_utils.function_name`.
'''
...
def log_resource_usage(interval=0.1, output_file='resource_usage_log.csv'):
    ...
    Args:
        ...
        output_file (str, optional):The path to the output CSV file where 
                                    resource usage data will be saved. 
                                    Defaults to "resource_usage_log.csv".
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
                        ...
                        for child in current_process.children():
                            ...
                            for grandchild in child.children(recursive=True):
                                if grandchild.is_running():
                                    ...
                                    ps_cpu_time = grandchild.cpu_times()
                                    ps_threads_usage += grandchild.num_threads()
                                    ps_memory_usage += child.memory_full_info().rss
                                    ps_disk_io = child.io_counters()
                                    ps_disk_read += ps_disk_io.read_bytes
                                    ps_disk_write += ps_disk_io.write_bytes
                            ...
                            bg_disk_read = disk_io.read_bytes                                   # pyright: ignore[reportOptionalMemberAccess]
                            bg_disk_write = disk_io.write_bytes                                 # pyright: ignore[reportOptionalMemberAccess]
                            # Append resource usage to the list
                            log_time = round(time.time() - t_start, 3)
                            resource_usage_data.append((
                                log_time, ps_name, ps_threads_usage, ps_cpu_usage,
                                ps_memory_usage, ps_disk_read, ps_disk_write, bg_cpu_usage,
                                bg_memory_usage, bg_disk_read, bg_disk_write
                            ))
            ...
            try:
                ...
                data.to_csv(f'{self.output_dir}/{output_file}', index=False)
